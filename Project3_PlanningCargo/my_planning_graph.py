...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        self.a_levels = []
        self.create_graph()
        self.state_map = problem.state_map
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            new_node = PgNode_a(action)                     # creates a node for each action
            if all(node in self.s_levels[level] for node in new_node.prenodes):     # test of precond of action node all subset of S level
                for prenode in new_node.prenodes:           # double loop to add children / parent
                    for s_node in self.s_levels[level]:
                        if prenode == s_node:
                            new_node.parents.add(s_node)
                            s_node.children.add(new_node)
                self.a_levels[level].add(new_node)          # add action node
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
            for literal_node in a_node.effnodes:
                literal_node.parents.add(a_node)
                a_node.children.add(literal_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect1 in node_a1.effnodes:
            for precon2 in node_a2.prenodes:
                if (effect1.symbol == precon2.symbol) and (effect1.is_pos != precon2.is_pos):
                    return True

        for effect2 in node_a2.effnodes:
            for precon1 in node_a1.prenodes:
                if (effect2.symbol == precon1.symbol) and (effect2.is_pos != precon1.is_pos):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        if all(parent1.is_mutex(parent2) for parent1 in node_s1.parents for parent2 in node_s2.parents):
            return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            while check is False:
                for node in self.s_levels[counter]:
    ...
    #             for s_node in self.s_levels[level]:     # looking for the goal among s_nodes
    #                 if goal == s_node.literal:  # if goal found at this level, then add level cost to level_sum
    #                     level_sum += level
    #                     goal_found = True
    #                     break
    #             if goal_found:
    #                 break       # go to the next goal
    #
    #     return level_sum